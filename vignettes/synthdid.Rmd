...
::: {.callout-note}
## New Formula Interface Available!

**Version 2.0** introduces a modern formula-based interface similar to `lm()` and `plm()`.

For the new interface, see `vignette("formula-interface")`. This vignette demonstrates the classic matrix-based interface, which remains fully supported.

**Quick comparison:**

```r
# Classic interface (this vignette)
...
# Example use
We estimate the effect of California Proposition 99 on cigarette consumption using the **classic matrix-based interface**. For the modern formula interface, see `vignette("formula-interface")`.
