...
This vignette uses the classic matrix-based interface. All plotting functions work identically with the new formula interface introduced in Version 2.0. See `vignette("formula-interface")` for details on the modern interface.

```r
# Both interfaces work with all plotting functions
result <- synthdid(PacksPerCapita ~ treated, data, index = c("State", "Year"))
plot(result)  # Same plots!
```
:::
