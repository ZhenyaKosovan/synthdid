...
#' Randomize treatment assignment with sensible caps
#'
#' Draws binary assignments with probability \code{pi} and enforces at least one
#' treated unit and no more than \code{N1}. Excess treated units are dropped at
#' random; if none are treated, units are promoted at random.
#'
#' @param pi Randomization probabilities.
#' @param N Number of units.
#' @param N1 Maximum number of treated units to allow.
#'
#' @return A binary vector of length \code{N} indicating treatment assignment.
#' @keywords internal
...
#' Decompose outcomes into signal and noise components
#'
#' Splits an outcome matrix into low-rank signal \code{L = F + M} and residual
#' \code{E} using the leading singular vectors, as described in Section 3.1.1 of
#' the paper.
#'
#' @param Y Outcome matrix.
#' @param rank Assumed rank of the signal component \code{L}.
#'
#' @return A list with elements \code{F}, \code{M}, \code{E}, and
#'   \code{unit_factors}.
#' @keywords internal
...
#' Estimate AR(2) coefficients from panel residuals
#'
#' Fits a simple autoregressive model to the residual matrix produced by
#' \code{decompose_Y}.
#'
#' @param E Matrix whose rows are time series.
#'
#' @return Numeric vector of AR(2) coefficients \code{c(phi1, phi2)}.
#' @keywords internal
fit_ar2 <- function(E){
...
#' Build the correlation matrix implied by an AR(2) process
#'
#' @param ar_coef Vector of AR(2) coefficients \code{c(phi1, phi2)}.
#' @param T Length of the time series.
#'
#' @return Correlation matrix for the implied AR(2) process.
#' @keywords internal
